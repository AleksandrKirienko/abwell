...
class EventHandler::BuffRequest
  attr_reader :message_text, :message_id, :sender_id, :bot_chat_id

  def initialize(message_text, message_id, sender_id, bot_chat_id)
  ...
  def call
    ...
    notify_missing_buffs(missing_buffs, message_id, bot_chat_id)
  ...
  def create_game_account_if_needed
    GameAccount.where(vk_id: @sender_id).first_or_create
  end
  ...
  def notify_missing_buffs(missing_buffs, message_id, bot_chat_id)
    return if missing_buffs.empty?

    MissingBuffsNotifier.new(missing_buffs, message_id, bot_chat_id).call
  end
end
