...
RSpec.describe EventHandler::BuffResult::Unsuccessful::RaceBuffInEffect do
  subject(:handler) { described_class.new(message_text, chat_id) }

  let(:chat_id) { 111 }
  let(:vk_id) { 37606453 }
  let(:message_text) { "🚫[id#{vk_id}|Александр], на цель уже наложено другое расовое благословение!" }
